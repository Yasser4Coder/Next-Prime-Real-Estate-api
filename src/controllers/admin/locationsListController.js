...
const PURPOSE_VALUES = ['buy', 'rent', 'off-plan']
...
async function list(req, res) {
  try {
    ...
    const offPlan = rows.filter((l) => l.purpose === 'off-plan').map((l) => l.name)
    return res.json({ buy, rent, offPlan })
  } catch (err) {
    console.error(err)
...
async function add(req, res) {
  try {
    ...
    const purpose = PURPOSE_VALUES.includes(req.body.purpose) ? req.body.purpose : 'buy'
    ...
    const rent = rows.filter((l) => l.purpose === 'rent').map((l) => l.name)
    const offPlan = rows.filter((l) => l.purpose === 'off-plan').map((l) => l.name)
    return res.status(201).json({ buy, rent, offPlan })
  } catch (err) {
    console.error(err)
...
async function remove(req, res) {
  try {
    ...
    const purpose = PURPOSE_VALUES.includes(req.query.purpose) ? req.query.purpose : 'buy'
